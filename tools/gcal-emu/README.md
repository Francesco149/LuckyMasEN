...
> **Retired** — superseded by the native **[`../gcal-xp/`](../gcal-xp/README.md)** server (the one the
> end-user runs on XP; it also speaks the HTTPS ClientLogin this board never grew). Kept as the
> human-readable **protocol oracle**.
...
> ⚠️ **Run on a separate always-on box, not the XP box itself** — the XP box runs one OS at a time, so
> it's offline while XP is booted (and XP reuses its NIC/lease). Needs **port 80 free**; XP's hosts
> points `www.google.com` at *this* box's IP.
...
1. `C:\WINDOWS\system32\drivers\etc\hosts` → add `<emu-host-ip>  www.google.com` (the separate box
   running this emulator). ⚠️ Also blackholes *real* google.com browsing on XP — fine for a retro box;
   remove the line to restore.
...
## Status — retired (kept as the protocol oracle)

Superseded by the native **[`../gcal-xp/`](../gcal-xp/README.md)** server, which the end-user actually
runs on XP and which also speaks the **HTTPS ClientLogin** this Python board never grew. Self-tested on
all scenarios (curl + a POP3 client); the live `gcal.exe` validation happened against the native server.
This board is kept only as the human-readable **protocol oracle** — the exact responses the launcher expects.
